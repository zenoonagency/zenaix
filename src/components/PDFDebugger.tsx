...
import { Eye, FileText, Download, X, ChevronDown, ChevronUp, Code } from 'lucide-react';
...
export function PDFDebugger({ fileUrl, onClose }: PDFDebuggerProps) {
  ...
  const analyze = () => {
    ...
    const isExternalUrl = isUrl && !isBlobUrl && !isDataUrl && !fileUrl.startsWith(window.location.origin);

    // Verificar se parece ser base64 puro
    const isPureBase64 = /^[A-Za-z0-9+/=]+$/.test(fileUrl) && fileUrl.length > 100;
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[9999] p-4 !mt-0">
      <div className={`w-full max-w-3xl ${isDark ? 'bg-dark-800 text-gray-200' : 'bg-white text-gray-800'} rounded-lg shadow-xl overflow-hidden`}>
        <div className={`flex items-center justify-between p-4 border-b ${isDark ? 'border-dark-600' : 'border-gray-200'}`}>
          ...
          <button
            onClick={onClose}
            className={`${isDark ? 'hover:bg-dark-700' : 'hover:bg-gray-100'} p-1 rounded-full`}
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className={`p-4 ${isDark ? 'bg-dark-700' : 'bg-gray-50'}`}>
          <div className="space-y-2">
            ...
            <div className={`p-3 rounded ${isDark ? 'bg-dark-800' : 'bg-white'} space-y-2 text-sm`}>
              <div className="grid grid-cols-2 gap-2">
                <div>Aparenta ser URL:</div>
                <div className={analysis.isUrl ? 'text-green-500' : 'text-red-500'}>
                  {analysis.isUrl ? 'Sim' : 'Não'}
                </div>

                <div>Data URL:</div>
                <div className={analysis.isDataUrl ? 'text-green-500' : 'text-gray-500'}>
                  {analysis.isDataUrl ? 'Sim' : 'Não'}
                </div>

                <div>Blob URL:</div>
                <div className={analysis.isBlobUrl ? 'text-green-500' : 'text-gray-500'}>
                  {analysis.isBlobUrl ? 'Sim' : 'Não'}
                </div>

                <div>URL externa:</div>
                <div className={analysis.isExternalUrl ? 'text-yellow-500' : 'text-gray-500'}>
                  {analysis.isExternalUrl ? 'Sim' : 'Não'}
                </div>

                <div>Base64 puro:</div>
                <div className={analysis.isPureBase64 ? 'text-yellow-500' : 'text-gray-500'}>
                  {analysis.isPureBase64 ? 'Sim' : 'Não'}
                </div>

                <div>Reconhecido como PDF:</div>
                <div className={analysis.recognizedAsPdf ? 'text-green-500' : 'text-red-500'}>
            ...
            <div className={`mt-3 p-3 rounded ${isDark ? 'bg-dark-800' : 'bg-white'}`}>
            ...
            <div className="mt-4">
              <button
                ...
                className={`flex items-center text-sm ${isDark ? 'text-gray-300 hover:text-white' : 'text-gray-700 hover:text-black'}`}
              >
              ...
              {showRawData && (
                <div className={`mt-2 p-3 rounded ${isDark ? 'bg-dark-900' : 'bg-gray-100'} relative`}>
            ...
            {analysis.isDataUrl && (
              <div className="mt-2">
                <button
                  ...
                  className={`flex items-center text-sm ${isDark ? 'text-gray-300 hover:text-white' : 'text-gray-700 hover:text-black'}`}
                >
                  {showBase64 ? <ChevronUp className="w-4 h-4 mr-1" /> : <ChevronDown className="w-4 h-4 mr-1" />}
                  {showBase64 ? 'Ocultar Dados Base64' : 'Mostrar Dados Base64'}
                </button>
                
                {showBase64 && (
                  <div className={`mt-2 p-3 rounded ${isDark ? 'bg-dark-900' : 'bg-gray-100'}`}>
                    <pre className="text-xs overflow-auto max-h-24 whitespace-pre-wrap break-all">
