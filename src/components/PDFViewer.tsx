...
import { Download, RefreshCw, Info, ExternalLink, FileText, AlertTriangle } from 'lucide-react';
...
export function PDFViewer({ 
  ...
  useEffect(() => {
    if (fileUrl) {
      ...
      if (!isPdfUrl(fileUrl) && !fileUrl.startsWith('blob:') && !fileUrl.startsWith('data:')) {
  ...
  const loadPdf = async () => {
    ...
    try {
      console.log("PDFViewer - Tentando carregar PDF da URL:", fileUrl.substring(0, 100) + (fileUrl.length > 100 ? "..." : ""));
      ...
      if (fileType === "base64-pdf" || fileType === "base64-other" || fileType === "blob") {
        // Usar diretamente
      ...
      setDebugInfo({
        ...
        proxiedUrl: url,
        urlType: fileType,
        method: url.includes('docs.google.com/viewer') ? "googleViewer" : 
                url.startsWith('blob:') ? "blob" : "direct"
      });
      
      setLoading(false);
    } catch (error) {
      console.error('Erro ao processar URL do PDF:', error);
      setError('Não foi possível carregar o PDF. Erro: ' + (error instanceof Error ? error.message : String(error)));
      setLoading(false);
    }
  };
  ...
  const forceDirectView = async () => {
    ...
    try {
      ...
      if (fileUrl.length > 500 && /^[A-Za-z0-9+/=]+$/.test(fileUrl)) {
        console.log("PDFViewer - Detectada possível string base64 não formatada");
        // Tenta converter base64 raw para data URL
        const wrappedBase64 = `data:application/pdf;base64,${fileUrl}`;
        setProxiedUrl(wrappedBase64);
        setDebugInfo({
  ...
  const handleDownload = async () => {
    try {
      ...
      const { blobUrl, fileName: downloadFileName } = await pdfProxyService.getPdfForDownload(fileUrl, fileName);
  ...
  const handleIframeError = () => {
    ...
    setError("Não foi possível exibir o PDF. Tentando visualização alternativa...");
    // Tentar fallback
    switchViewMode();
  };

  // Verificação adicional de URL
  const isValidPdfUrl = Boolean(proxiedUrl && (
    ...
    >
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center bg-white/80 dark:bg-black/50 z-10">
          <div className="flex flex-col items-center">
            ...
            <p className="mt-3 text-sm text-gray-600 dark:text-gray-300">Carregando PDF...</p>
          </div>
        </div>
      )}
      ...
      {showControls && !error && (
        <div className="absolute top-2 right-2 flex gap-1 z-20">
          ...
          <button
            ...
            title={fallbackMode ? "Usar visualização direta" : "Usar Google Viewer"}
          >
            <RefreshCw size={16} />
          </button>
          <button
      ...
      {showDebugInfo && (
        <div className="absolute top-10 right-2 bg-white dark:bg-dark-800 shadow-lg rounded p-3 z-30 text-xs overflow-auto max-w-xs max-h-[300px] border border-gray-200 dark:border-dark-600">
          <h4 className="font-bold mb-2 text-gray-800 dark:text-gray-200">Informações de Depuração</h4>
          <div className="space-y-1 text-gray-700 dark:text-gray-300">
            <p><strong>URL Original:</strong> {fileUrl ? (fileUrl.length > 50 ? fileUrl.substring(0, 50) + "..." : fileUrl) : "Nenhuma"}</p>
            <p><strong>URL Utilizada:</strong> {proxiedUrl ? (proxiedUrl.length > 50 ? proxiedUrl.substring(0, 50) + "..." : proxiedUrl) : "Nenhuma"}</p>
            <p><strong>Tipo de URL:</strong> {debugInfo.urlType || "Desconhecido"}</p>
            <p><strong>Método:</strong> {debugInfo.method || "Nenhum"}</p>
            <p><strong>Modo Fallback:</strong> {fallbackMode ? "Sim" : "Não"}</p>
            <p><strong>É PDF válido:</strong> {isValidPdfUrl ? "Sim" : "Não"}</p>
          </div>
        </div>
      )}
      ...
      {proxiedUrl && isValidPdfUrl ? (
        <iframe 
          ...
          sandbox={fallbackMode ? "allow-scripts allow-same-origin allow-forms" : ""}
        />
      ) : (
        <div className="w-full h-full flex flex-col items-center justify-center bg-gray-50 dark:bg-dark-900">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-center max-w-xs px-6">
            {loading ? "Carregando documento..." : "Erro ao carregar documento PDF"}
          </p>
          {!loading && (
            <button
              onClick={() => setShowDebugger(true)}
