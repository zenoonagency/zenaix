...
export const useKanbanStore = create<KanbanState & {
  ...
  persist(
    (set, get) => {
      ...
      const mappers = {
        boardToExtendedBoard: (board: SupaBoard & { lists?: (SupaList & { cards?: SupaCard[] })[] }): ExtendedBoard => {
        ...
        try {
          ...
          set((state) => ({
            boards: state.boards.map(board => 
              board.id === id ? mappers.boardToExtendedBoard(boardDetails) : board
            )
          }));
        } catch (error) {
          console.error(`[KanbanStore] Erro ao carregar detalhes do board ${id}:`, error);
        }
      ...
      return {
        ...
        fetchBoardDetails: async (id) => {
          try {
            ...
            console.error('[KanbanStore] Erro ao carregar detalhes do quadro:', error);
            set({ isLoading: false, error: error as Error });
        ...
        addList: async (boardId, title) => {
          try {
            ...
            const newList = await kanbanService.createList(boardId, title, position);
              ...
              boards: state.boards.map(board => {
                ...
                return { 
                  ...
                  lists: [...(board.lists || []), { 
                    ...newList, 
                    cards: [],
                    // Adicionar propriedades específicas do aplicativo
                    id: newList.id,
            ...
            console.error('[KanbanStore] Erro ao criar lista:', error);
            throw error;
          }
        },
        ...
        updateList: async (id, updates) => {
          try {
            ...
            set((state) => ({
              boards: state.boards.map(board => {
                // Se o board não tem listas, retorna sem modificar
                if (!board.lists) return board;
                
                // Se o board contém a lista atualizada
                  ...
                  return {
                    ...
                    lists: board.lists.map(list => 
                      list.id === id ? { 
                        ...list, 
                        ...updatedList,
                        // Garantir que as propriedades específicas sejam mantidas
            ...
            console.error('[KanbanStore] Erro ao atualizar lista:', error);
            throw error;
          }
        },
          ...
          try {
            ...
            set((state) => ({
              boards: state.boards.map(board => {
                ...
                // Se o board contém a lista, removê-la
                if (board.lists.some(list => list.id === id)) {
                  return {
                    ...board,
            ...
            console.error('[KanbanStore] Erro ao excluir lista:', error);
            throw error;
          }
        },
        ...
        addCard: async (listId, title, description) => {
          try {
            ...
            const newCard = await kanbanService.createCard(listId, title, listPosition, description);
        ...
        setCompletedList: (boardId, listId) => {
          ...
          localStorage.setItem('kanbanCompletedLists', JSON.stringify(localSettings.completedLists));
        },
        ...
        toggleBoardVisibility: (id) => set((state) => ({
          boards: state.boards.map((board) =>
