...
import { Send, History, User, Zap, Bug, RefreshCw, AlertCircle, X, SmartphoneNfc, QrCode, ArrowRight, Phone, Trash2 } from 'lucide-react';
...
const DISPARO_WEBHOOK = 'https://fluxos-n8n.mgmxhs.easypanel.host/webhook/disparo';
...
export function Messaging() {
  ...
  const { name: userName, phoneNumber: userPhone } = useUser();
  ...
  useEffect(() => {
    ...
    try {
      ...
      try {
        ...
        console.error('Erro crítico de armazenamento, não foi possível limpar:', storageError);
      }
    }
  ...
  }, [userName, userPhone, whatsAppIsConnected, showToast, cleanupOldBatches, clearAllBatches]);
  ...
  const checkWhatsAppConnectionWithServer = async () => {
    try {
      const response = await fetch('https://fluxos-n8n.mgmxhs.easypanel.host/webhook/check-whatsapp-connection');
  ...
  const sendMessages = async (
    ...
    const largeMediaMessages = messages.filter(msg => 
      msg.type !== 'text' && 
      msg.content && 
    ...
    if (largeMediaMessages.length > 0) {
      showToast(`${largeMediaMessages.length} arquivo(s) muito grande(s). O tamanho máximo é de 10MB por arquivo.`, 'error');
      return;
    }
    ...
    if (!whatsAppIsConnected) {
      ...
      try {
        const checkResponse = await fetch('https://fluxos-n8n.mgmxhs.easypanel.host/webhook/check-whatsapp-connection');
        const checkData = await checkResponse.json();
        
        if (checkData.resposta === "whatsapp não conectado corretamente") {
          setShowWhatsAppConnectModal(true);
          showToast('Seu WhatsApp não está conectado. Configure a conexão para continuar.', 'error');
          return;
        }
      } catch (error) {
        console.error('Erro ao verificar conexão do WhatsApp:', error);
    ...
    const selectedContacts = manualContacts.filter(c => contactIds.includes(c.id));
    ...
    addBatch(batchId, context, messages, selectedContacts);
        ...
        console.warn('Aviso: Histórico desativado devido a limites de armazenamento', storageError);
      }
      ...
      console.log('Enviando payload para o webhook:', JSON.stringify({
      ...
      try {
        ...
        if (responseData && responseData.resposta === "sucesso") {
          ...
          console.log(`Mensagens enviadas com sucesso para ${selectedContacts.length} contato(s)`);
          showToast(`Disparo realizado com sucesso para ${selectedContacts.length} contato(s)`, 'success');
        } else if (responseData && responseData.resposta === "whatsapp não conectado corretamente") {
          // Caso de WhatsApp não conectado
          failedCount = selectedContacts.length; // Todos falharam
          console.error(`Falha ao enviar mensagens: WhatsApp não conectado corretamente`);
          
          // Exibir notificação sobre o problema de conexão do WhatsApp
          showToast('WhatsApp não está conectado corretamente. Verifique sua conexão do WhatsApp.', 'error');
          setShowWhatsAppConnectModal(true);
        } else if (responseData && responseData.message === "There was a problem executing the workflow") {
          ...
          showToast('Erro no processamento do disparo: problema na execução do fluxo de trabalho. Verifique os tipos de mídia e tente novamente.', 'error');
          ...
          showToast('Alguns arquivos são muito grandes para processamento. O limite é de 10MB por arquivo. Reduza o tamanho das mídias.', 'error');
        } else if (responseData && responseData.message?.includes("Invalid media")) {
          // Erro de formato de mídia inválido
          failedCount = selectedContacts.length;
          console.error(`Erro de formato de mídia no webhook:`, responseData);
          
          showToast('Formato de mídia inválido. Verifique se os arquivos estão em formatos suportados pelo WhatsApp.', 'error');
        } else {
          // Outros tipos de erro na resposta
          failedCount = selectedContacts.length;
          console.error(`Falha ao enviar mensagens:`, `Resposta inesperada:`, responseData);
          
          // Mensagem amigável para o usuário
          showToast('Falha no envio das mensagens. Verifique os tipos de arquivo e tente novamente.', 'error');
        }
      } catch (parseError) {
        ...
        if (response.ok) {
          ...
          console.log(`Mensagens possivelmente enviadas com sucesso, mas resposta não é JSON válido`);
        } else {
          failedCount = selectedContacts.length;
          ...
          showToast(`Erro no servidor (${response.status}). Tente novamente mais tarde.`, 'error');
        }
      }
      ...
      console.error(
        ...
        '\nURL do webhook:', DISPARO_WEBHOOK,
        '\nDetalhes do payload:', JSON.stringify({
      ...
      showToast('Erro ao enviar mensagens. Verifique sua conexão com a internet.', 'error');
    } finally {
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-dark-1000' : 'bg-transparent'} p-6 relative`}>
      ...
      {showConfirmClearHistory && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-dark-800 rounded-lg shadow-xl w-full max-w-md p-6">
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-3">Limpar histórico</h3>
            <p className="text-gray-600 dark:text-gray-300 mb-5">
              Tem certeza que deseja limpar todo o histórico de disparos? Esta ação não pode ser desfeita.
            </p>
      ...
      <div className={`transition-all duration-300 ${showWhatsAppConnectModal ? 'blur-sm' : ''}`}>
        ...
        <div className="mb-8 relative">
          ...
          {(!userName || !userPhone) && (
            <div className={`mb-6 ${isDark ? 'bg-dark-750 border-yellow-600' : 'bg-yellow-50 border-yellow-500'} border-l-4 rounded-lg p-4 shadow-md`}>
              <div className="flex items-start">
                <AlertCircle className="w-6 h-6 text-yellow-500 mt-0.5 mr-3 flex-shrink-0" />
                <div>
                  <h3 className={`text-sm font-medium ${isDark ? 'text-yellow-400' : 'text-yellow-800'}`}>Configuração Necessária</h3>
                  <p className={`mt-1 text-sm ${isDark ? 'text-yellow-300' : 'text-yellow-700'}`}>
                    Para utilizar o disparo em massa, você precisa configurar seu nome e número de telefone no perfil.
                  </p>
                  <div className="mt-3">
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 relative z-10">
        ...
        <div className="lg:col-span-8 space-y-6">
            <div className={`${isDark ? 'bg-dark-800 border-dark-700' : 'bg-gray-50 border-gray-200'} rounded-xl shadow-xl border overflow-hidden`}>
              <div className={`p-4 border-b ${isDark ? 'border-dark-700' : 'border-gray-200'} flex items-center justify-between`}>
                <div className="flex items-center gap-2">
                  <Send className="w-5 h-5 text-[#7f00ff]" />
                  <h2 className={`font-medium ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>Disparo em massa</h2>
                </div>
                {userName && userPhone && (
                  <div className={`flex items-center gap-2 text-xs ${isDark ? 'bg-dark-700/50 text-gray-400' : 'bg-gray-100 text-gray-600'} py-1 px-2 rounded`}>
                  </div>
                )}
              </div>
              <div className={`p-6 ${isDark ? 'bg-dark-800' : 'bg-gray-50'}`}>
                <div className="flex justify-between items-center mb-8">
                    <h1 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>Disparo em Massa</h1>
                  <div className="flex items-center gap-4">
                    ...
                    <div className={`flex items-center gap-2 px-3 py-2 rounded-lg ${
                      ...
                      <span className="text-sm font-medium">
                        {whatsAppIsConnected ? 'WhatsApp Conectado' : 'WhatsApp Desconectado'}
                      </span>
                      {!whatsAppIsConnected && (
                        <button 
                          onClick={() => setShowProfileModal(true)} 
          ...
          <div className="lg:col-span-4">
            <div className={`${isDark ? 'bg-dark-800 border-dark-700' : 'bg-gray-50 border-gray-200'} rounded-xl shadow-xl border overflow-hidden sticky top-6`}>
              <div className={`p-4 border-b ${isDark ? 'border-dark-700' : 'border-gray-200'} flex items-center justify-between`}>
                <div className="flex items-center gap-2">
                  <History className="w-5 h-5 text-[#7f00ff]" />
                  <h2 className={`font-medium ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>Histórico</h2>
                </div>
              ...
              <div className={`max-h-[calc(100vh-220px)] overflow-y-auto scrollbar-thin ${isDark ? 'bg-dark-800' : 'bg-gray-50'}`}>
      ...
      {showWhatsAppConnectModal && (
        <div className={`fixed inset-0 ${
          ...
          <div className={`${isDark ? 'bg-dark-800' : 'bg-white'} rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-300`}>
            <div className={`p-4 border-b ${isDark ? 'border-dark-700' : 'border-gray-200'} flex items-center justify-between`}>
              <div className="flex items-center gap-2">
              ...
              <button 
                ...
                className={`p-1 rounded-full ${isDark ? 'hover:bg-dark-700' : 'hover:bg-gray-100'} transition-colors focus:outline-none`}
              >
            ...
            <div className={`p-6 ${isDark ? 'bg-dark-800' : 'bg-white'}`}>
              <div className="mb-6 text-center">
                <h3 className={`text-xl font-semibold ${isDark ? 'text-white' : 'text-gray-800'} mb-2`}>Seu WhatsApp não está conectado</h3>
                <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Siga os passos abaixo para conectar seu WhatsApp e começar a enviar mensagens:</p>
              </div>
              
              {/* Carrossel de imagens demonstrativas */}
              <div className={`relative mb-8 ${
                isDark ? 'bg-dark-750 border-dark-600' : 'bg-gray-50 border-gray-200'
                ...
                <div 
                  ...
                  onScroll={(e) => {
                    ...
                    if (activeIndex !== currentSlide && activeIndex >= 0 && activeIndex <= 3) {
                      setCurrentSlide(activeIndex);
                    }
                  }}
                >
                  {/* Slide 1 */}
                  <div className={`snap-center min-w-full flex flex-col items-center p-2 ${currentSlide === 0 ? 'slide-active' : ''}`}>
                    <img 
                    ...
                    />
                    <h4 className={`font-medium text-lg mt-2 ${isDark ? 'text-white' : 'text-gray-800'}`}>Passo 1</h4>
                    <p className={`text-sm text-center ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Clique em editar perfil</p>
                  </div>
                  
                  {/* Slide 2 */}
                  <div className={`snap-center min-w-full flex flex-col items-center p-2 ${currentSlide === 1 ? 'slide-active' : ''}`}>
                    <img 
                      ...
                      className="max-w-full h-auto rounded-lg shadow-lg mb-4 max-h-96 object-contain"
                    />
                    <h4 className={`font-medium text-lg mt-2 ${isDark ? 'text-white' : 'text-gray-800'}`}>Passo 2</h4>
                    <p className={`text-sm text-center ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Preencha o nome e número e clique em conectar whatsapp</p>
                  </div>
                  
                  {/* Slide 3 */}
                  <div className={`snap-center min-w-full flex flex-col items-center p-2 ${currentSlide === 2 ? 'slide-active' : ''}`}>
                    <img 
                      ...
                      className="max-w-full h-auto rounded-lg shadow-lg mb-4 max-h-96 object-contain"
                    />
                    <h4 className={`font-medium text-lg mt-2 ${isDark ? 'text-white' : 'text-gray-800'}`}>Passo 3</h4>
                    <p className={`text-sm text-center ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Faça a leitura do QRCODE com o número que quer efetuar o disparo</p>
                  </div>
                  
                  {/* Slide 4 */}
                  <div className={`snap-center min-w-full flex flex-col items-center p-2 ${currentSlide === 3 ? 'slide-active' : ''}`}>
                    <img 
                      ...
                      className="max-w-full h-auto rounded-lg shadow-lg mb-4 max-h-96 object-contain"
                    />
                    <h4 className={`font-medium text-lg mt-2 ${isDark ? 'text-white' : 'text-gray-800'}`}>Passo 4</h4>
                    <p className={`text-sm text-center ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Após feito o passo a passo, deve aparecer dessa forma</p>
                  </div>
                </div>
