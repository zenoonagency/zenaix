...
import { ChevronLeft, Lightbulb, Book, Video, Coffee, ChevronDown, ChevronUp, ExternalLink, Info, Star, MessageSquare, UserPlus, Users, LineChart, MessageCircle, Smile, Loader } from 'lucide-react';
...
const tutorialVideos: TutorialVideo[] = [
  {
    ...
    description: 'Conheça as principais funcionalidades e recursos do sistema Zenaix',
  ...
  },
  {
    ...
    description: 'Aprenda a configurar e enviar mensagens em massa pelo WhatsApp',
  ...
  },
  {
    ...
    description: 'Como importar, organizar e gerenciar seus contatos no sistema',
  ...
  },
  {
    ...
    description: 'Passo a passo para configurar e personalizar seu funil de vendas',
  ...
  },
  {
    ...
    description: 'Como aproveitar ao máximo o assistente de inteligência artificial',
...
function VideoAccordion({ video }: { video: TutorialVideo }) {
  ...
  return (
    <div className={`border ${isDark ? 'border-dark-700' : 'border-gray-200'} rounded-lg mb-3 overflow-hidden transition-all`}>
      <button
        onClick={() => setIsOpen(!isOpen)}
        className={`w-full flex items-center justify-between p-4 text-left ${
          isDark 
            ? isOpen ? 'bg-dark-700' : 'bg-dark-800 hover:bg-dark-750' 
      ...
      >
        <div className="flex items-center">
          <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-3 ${
            ...
            {video.icon || <Video className={`${isDark ? 'text-[#7f00ff]/80' : 'text-[#7f00ff]'}`} />}
          </div>
          <div>
        ...
        {isOpen ? 
          <ChevronUp className={`w-5 h-5 ${isDark ? 'text-gray-400' : 'text-gray-600'} transition-transform duration-300`} /> : 
          <ChevronDown className={`w-5 h-5 ${isDark ? 'text-gray-400' : 'text-gray-600'} transition-transform duration-300`} />
        }
      </button>
      ...
      >
        <div className={`p-4 ${isDark ? 'bg-dark-850 border-t border-dark-700' : 'bg-white border-t border-gray-200'}`}>
          <p className={`mb-4 ${isDark ? 'text-gray-300' : 'text-gray-700'} text-sm`}>
            {video.description}
          </p>
          <div className="rounded-lg overflow-hidden shadow-md" style={{ position: 'relative', paddingBottom: '56.25%', height: 0 }}>
            <iframe 
...
function VideoTabs({ activeCategory, setActiveCategory }: { 
          ...
          <button
            ...
            className={`py-2 px-4 font-medium text-sm border-b-2 -mb-px transition-colors ${
              activeCategory === category.id
                ? `border-[#7f00ff] ${isDark ? 'text-[#7f00ff]' : 'text-[#7f00ff]'}`
...
export function Help() {
  ...
  const [activeSection, setActiveSection] = useState<'videos' | 'service' | 'feedback'>('videos');
  ...
  const [contactType, setContactType] = useState<'service' | 'feedback'>('service');
  ...
  const handleServiceFormChange = (e: React.ChangeEvent<HTMLSelectElement | HTMLTextAreaElement>) => {
  ...
  const handleFeedbackFormChange = (e: React.ChangeEvent<HTMLSelectElement | HTMLTextAreaElement>) => {
    const { name, value } = e.target;
  ...
  const handleContactSubmit = async (contactData: ContactData) => {
    const formData = contactType === 'service' ? serviceFormData : feedbackFormData;
  ...
  return (
    <div className={`min-h-screen p-6 bg-transparente`}>
          ...
          >
            <div className="flex flex-col items-center">
              <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${
                activeSection === 'videos'
                  ? isDark ? 'bg-[#7f00ff]/40' : 'bg-[#7f00ff]/20'
                  : isDark ? 'bg-[#7f00ff]/20' : 'bg-[#7f00ff]/10'
              }`}>
                <Video className={`w-8 h-8 ${
                  activeSection === 'videos' ? 'text-[#7f00ff]' : 'text-[#7f00ff]/80'
                }`} />
              </div>
              ...
              <p className={`text-center ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                Assista a vídeos explicativos sobre as funcionalidades do sistema.
              </p>
            </div>
          </div>
            ...
            <div className="flex flex-col items-center">
              <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${
                ...
                <MessageCircle className={`w-8 h-8 ${
                  activeSection === 'service' ? 'text-[#7f00ff]' : 'text-[#7f00ff]/80'
                }`} />
              </div>
              <h2 className={`text-xl font-semibold mb-3 ${
                activeSection === 'service'
              ...
              <p className={`text-center ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                Solicite suporte técnico ou um serviço personalizado para seu negócio.
              </p>
            </div>
          </div>
            ...
            <div className="flex flex-col items-center">
              <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${
                ...
                <Smile className={`w-8 h-8 ${
                  activeSection === 'feedback' ? 'text-[#7f00ff]' : 'text-[#7f00ff]/80'
                }`} />
              </div>
              <h2 className={`text-xl font-semibold mb-3 ${
                activeSection === 'feedback'
        ...
        <div className={`rounded-xl p-6 ${
          ...
          {activeSection === 'videos' && (
            <div>
              ...
              <VideoTabs activeCategory={activeCategory} setActiveCategory={setActiveCategory} />
              ...
              <div>
                {filteredVideos.length > 0 ? (
                  ...
                  <div className={`py-6 text-center ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                    <Video className={`w-12 h-12 mx-auto mb-3 ${isDark ? 'text-gray-600' : 'text-gray-400'}`} />
              ...
              <form className="space-y-4" onSubmit={handleSubmitServiceForm}>
                <div>
                  ...
                  >
                    ...
                    <option value="Criação de agente de IA">Criação de agente de IA</option>
                    <option value="Integração da IA com o Zenaix">Integração da IA com o Zenaix</option>
                    <option value="Treinamento de equipe">Treinamento de equipe</option>
                    <option value="Serviços de marketing">Serviços de marketing</option>
                    <option value="Criação de sistema">Criação de sistema</option>
                  ...
                  <div className="flex gap-4">
                    {['Baixa', 'Média', 'Alta', 'Urgente'].map((priority) => (
                      <label key={priority} className="flex items-center">
                        ...
                        />
                        <span className={`${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{priority}</span>
                      </label>
                    ))}
                  </div>
                </div>
              ...
              <form className="space-y-4" onSubmit={handleSubmitFeedbackForm}>
                <div>
                  ...
                  >
                    ...
                    <option value="Nova funcionalidade">Sugerir uma nova funcionalidade</option>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                      {feedbackFormData.avaliacao > 0 
                        ? `${feedbackFormData.avaliacao} estrela${feedbackFormData.avaliacao !== 1 ? 's' : ''}` 
                        : 'Clique para avaliar'}
                    </span>
                  </div>
                </div>
      ...
      <ContactModal
        ...
        title={contactType === 'service' ? 'Dados para contato' : 'Seus dados de contato'}
        type={contactType}
        formData={contactType === 'service' ? serviceFormData : feedbackFormData}
      />
    </div>
  );
} 