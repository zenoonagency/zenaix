...
const MoveCardModal = React.memo(({ isOpen, onClose, onMove, lists, currentListId }: MoveCardModalProps) => {
...
const Card = React.memo(({ card, boardId, listId, isDragging, className }: CardProps) => {
  ...
  const handleSaveEdit = useCallback((updatedCard: Partial<CardType>) => {
  ...
  const handleMove = useCallback((targetListId: string) => {
  ...
  const cardClassName = useMemo(() => `
    ...
    ${isDragging ? 'cursor-grabbing opacity-50' : 'cursor-grab hover:border-[#7f00ff] dark:hover:border-[#7f00ff] opacity-100'}
    ${className || ''}
          ...
          <div className="space-y-2">
            ...
            {cardData.dueDate && (
              <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                <span>
                  {format(new Date(cardData.dueDate), 'dd MMM yyyy', { locale: ptBR })}
                </span>
              </div>
            )}
            ...
            {cardData.priority && (
              <div className="flex items-center gap-2">
                <span className={`inline-flex items-center gap-1 px-2 py-1 text-xs rounded-full ${PRIORITY_COLORS[cardData.priority as keyof typeof PRIORITY_COLORS]}`}>
                  {PRIORITY_ICONS[cardData.priority as keyof typeof PRIORITY_ICONS]}
                  <span>{cardData.priority.charAt(0).toUpperCase() + cardData.priority.slice(1)}</span>
                </span>
              </div>
            )}
