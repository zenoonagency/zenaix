...
import { X, Webhook, ArrowRight, Plus, Trash2, Tag as TagIcon } from 'lucide-react';
...
export function AutomationModal({ isOpen, onClose, boardId }: AutomationModalProps) {
  ...
  const saveAutomations = (newAutomations: Automation[]) => {
    localStorage.setItem(`automations_${boardId}`, JSON.stringify(newAutomations));
  ...
  const getTriggerDescription = (automation: Automation) => {
    ...
    switch (automation.triggerType) {
      case 'card_moved':
        return `Quando um cartão for movido de ${getListName(automation.sourceListId)} para ${getListName(automation.targetListId)}`;
      case 'card_created':
        return `Quando um cartão for criado em ${getListName(automation.sourceListId)}`;
      default:
                  ...
                  <div className="space-y-6">
                    ...
                    {automations.length === 0 ? (
                      <div className={`text-center py-12 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                        ...
                        <p className="text-sm mt-2">Clique em "Nova Automação" para começar</p>
                      </div>
                    ) : (
                        ...
                        {automations.map(automation => (
                          <div 
                            ...
                            className={`p-4 rounded-lg border ${
                              isDark 
                                ? automation.active ? 'border-gray-700 bg-dark-700' : 'border-gray-800 bg-dark-800 opacity-60' 
                                : automation.active ? 'border-gray-200 bg-white' : 'border-gray-200 bg-gray-50 opacity-60'
                            }`}
                          >
                              ...
                              <div>
                                ...
                                <div className="mt-2 flex items-center">
                                  ...
                                  <span className="text-xs text-gray-500 dark:text-gray-400 ml-3">
                                    Webhook: {automation.webhookUrl.substring(0, 30)}...
                                  </span>
                                </div>
                              </div>
                    ...
                    <div className="space-y-5 max-h-[60vh] overflow-y-auto pr-2">
                      ...
                      <div>
                        ...
                        >
                          ...
                          <option value="card_moved">Quando um cartão for movido entre listas</option>
                          <option value="card_created">Quando um cartão for criado em uma lista</option>
                        </select>
                      </div>
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => setCurrentAutomation({...currentAutomation, sourceListId: e.target.value})}
                              className={`w-full px-3 py-2 rounded-lg border ${
                                isDark 
                                  ? 'bg-dark-700 border-gray-600 text-gray-100' 
                                  : 'bg-white border-gray-300 text-gray-900'
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => setCurrentAutomation({...currentAutomation, targetListId: e.target.value})}
                              className={`w-full px-3 py-2 rounded-lg border ${
                                isDark 
                                  ? 'bg-dark-700 border-gray-600 text-gray-100' 
                                  : 'bg-white border-gray-300 text-gray-900'
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setCurrentAutomation({...currentAutomation, webhookUrl: e.target.value})}
                          className={`w-full px-3 py-2 rounded-lg border ${
                            isDark 
                              ? 'bg-dark-700 border-gray-600 text-gray-100' 
                              : 'bg-white border-gray-300 text-gray-900'
                      ...
                      <div className="flex items-center">
                        <input
                          ...
                          onChange={(e) => setCurrentAutomation({...currentAutomation, active: e.target.checked})}
                          className="w-4 h-4 text-[#7f00ff] border-gray-300 rounded focus:ring-[#7f00ff]"
                        />
                        <label htmlFor="active" className={`ml-2 text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
