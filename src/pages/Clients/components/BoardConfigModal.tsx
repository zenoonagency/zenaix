...
export function BoardConfigModal({ isOpen, onClose, boardId }: BoardConfigModalProps) {
  ...
  const handleVisibilityChange = (visibility: 'all' | 'creator' | 'selected') => {
    setConfig(prev => ({
      ...prev,
      visibility
    }));
            ...
            >
              <Dialog.Panel className="modal-content w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-dark-800 p-6 text-left align-middle shadow-xl transition-all">
                ...
                <div className="space-y-6 max-h-[60vh] overflow-y-auto pr-2">
                  <div>
                    <h4 className={`text-base font-medium mb-3 ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>
                    ...
                    <div className="space-y-2">
                      <label className={`flex items-center space-x-3 p-3 rounded-lg cursor-pointer ${
                        config.visibility === 'all' 
                          ? isDark ? 'bg-dark-700 border border-[#7f00ff]/50' : 'bg-purple-50 border border-[#7f00ff]/20' 
                          : isDark ? 'bg-dark-900 hover:bg-dark-700' : 'bg-gray-50 hover:bg-gray-100'
                      }`}>
                        <input
                          type="radio"
                          name="visibility"
                        ...
                        <div className={`flex items-center justify-center w-5 h-5 rounded-full ${
                          config.visibility === 'all' 
                            ...
                            : isDark ? 'bg-dark-600 border border-gray-600' : 'bg-white border border-gray-300'
                        }`}>
                          {config.visibility === 'all' && (
                        ...
                        {members.length === 0 ? (
                          ...
                          members.map(member => (
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={config.allowedUsers.includes(member.id)}
