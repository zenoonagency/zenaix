...
const checkAndExecuteAutomations = (
  ...
  try {
    ...
    const relevantAutomations = automations.filter((automation: any) => 
    ...
    (set, get) => ({
      ...
      deleteBoard: (boardId) => set((state) => {
        ...
        if (state.boards.length <= 1) {
          toast.warning('É necessário ter pelo menos um quadro.');
          return state;
        }

        const newBoards = state.boards.filter((board) => board.id !== boardId);
        const newActiveBoard = state.activeBoard === boardId ? newBoards[0].id : state.activeBoard;
      ...
      addCard: (boardId, listId, card) => {
        ...
        if (attachments.length > 5) {
          toast.error('Limite máximo de 5 anexos por cartão atingido');
          return;
        }
      ...
      updateCard: (boardId, listId, cardId, updatedCard) => {
        ...
        const currentCard = get().boards
          .find(b => b.id === boardId)
          ?.lists.find(l => l.id === listId)
          ?.cards.find(c => c.id === cardId);

        const newAttachments = updatedCard.attachments || currentCard?.attachments || [];
        ...
        // Verificar tamanho total dos anexos
        const totalSize = newAttachments.reduce((acc, curr) => acc + curr.size, 0);
        if (totalSize > 10 * 1024 * 1024) { // 10MB total
          toast.error('Tamanho total dos anexos excede 10MB');
          return;
        }
